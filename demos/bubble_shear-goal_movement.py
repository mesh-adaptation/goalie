...
def get_solver(mesh_seq):
    def solver(index):
        ...
        time, t_end = tp.subintervals[index]
...
def get_form(mesh_seq):
    def form(index, form_fields=None):
        Q = mesh_seq.function_spaces["c"][index]
        c, c_ = mesh_seq.fields["c"]
        time = mesh_seq.time_partition.subintervals[index][0]

        if form_fields is not None and "u" in form_fields:
            u, u_ = form_fields["u"]
        else:
            x, y = SpatialCoordinate(mesh_seq[index])
            V = VectorFunctionSpace(mesh_seq[index], "CG", 1)
