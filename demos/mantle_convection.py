...
# We must prescribe initial conditions to solve the problem. Note that goalie requires
# that initial condition are defined for all fields, including the mixed field ``up``
# despite the equations not involving a time derivative. In this case, the prescribed
# initial condition is used as the *initial guess* for the solver. Here, we set it to
# zero.
...
# The weak forms for the Stokes and energy equations are defined as follows.
...
# defined in the ``field_names`` list. This is to ensure that the error indicators are
# computed in the same order. Therefore, in the time integration loop we first solve the
# Stokes equations for the mixed velocity and pressure field, and then solve the energy
...
def get_solver(mesh_seq):
    def solver(index):
        ...
        # Boundary IDs and boundary conditions for both fields
        ...
        bcs_T = [
            DirichletBC(Q, 1.0, sub_domain=bottom),
            DirichletBC(Q, 0.0, sub_domain=top),
        ]
...
# Finally, we define the quantity of interest (QoI) as the heat flux over the top
# boundary of the domain, which has an id of 4. The QoI will be evaluated at the final
# timestep.


def get_qoi(mesh_seq, i):
    def qoi():
        T = mesh_seq.fields["T"][i]
        n = FacetNormal(mesh_seq.meshes[i])
        return dot(grad(T), n) * ds(4)
...
# *unsteady* (i.e. involves a time derivative). As already noted, the order of fields in
# ``field_types`` must match the order of fields in the ``fields_names`` list.
...
solutions, indicators = mesh_seq.indicate_errors(
    enrichment_kwargs={"enrichment_method": "p", "num_enrichments": 1}
)
...
fig.savefig("mantle_convection.jpg", bbox_inches="tight")
