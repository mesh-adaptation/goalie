import unittest

import numpy as np
import ufl
from firedrake.function import Function
from firedrake.functionspace import FunctionSpace
from firedrake.utility_meshes import UnitSquareMesh

from goalie.adjoint import AdjointSolver, annotate_qoi
from goalie.field import Field
from goalie.go_solver import GoalOrientedAdaptParameters
from goalie.mesh_seq import MeshSeq
from goalie.time_partition import TimeInterval


class TestAdjointUtils(unittest.TestCase):
    """
    Unit tests for adjoint utils.
    """

    def setUp(self):
        self.time_interval = TimeInterval(1.0, [0.5], Field("field", family="Real"))
        # self.mesh = UnitSquareMesh(1, 1)
        self.mesh_seq = MeshSeq(UnitSquareMesh(1, 1))

    def solver(self, qoi_type="end_time"):
        solver = AdjointSolver(self.time_interval, self.mesh_seq, qoi_type=qoi_type)
        solver.params = GoalOrientedAdaptParameters()
        return solver

    def test_annotate_qoi_0args(self):
        @annotate_qoi
        def get_qoi(solver, i):
            R = FunctionSpace(solver.meshes[i], "R", 0)

            def qoi():
                return Function(R).assign(1) * ufl.dx

            return qoi

        get_qoi(self.solver("end_time"), 0)

    def test_annotate_qoi_1arg(self):
        @annotate_qoi
        def get_qoi(solver, i):
            R = FunctionSpace(solver.meshes[i], "R", 0)

            def qoi(t):
                return Function(R).assign(1) * ufl.dx

            return qoi

        get_qoi(self.solver("time_integrated"), 0)

    def test_annotate_qoi_0args_error(self):
        @annotate_qoi
        def get_qoi(solver, i):
            R = FunctionSpace(solver.meshes[i], "R", 0)

            def qoi():
                return Function(R).assign(1) * ufl.dx

            return qoi

        with self.assertRaises(ValueError) as cm:
            get_qoi(self.solver("time_integrated"), 0)
        msg = "Expected qoi_type to be 'end_time' or 'steady', not 'time_integrated'."
        assert str(cm.exception) == msg

    def test_annotate_qoi_1arg_error(self):
        @annotate_qoi
        def get_qoi(solver, i):
            R = FunctionSpace(solver.meshes[i], "R", 0)

            def qoi(t):
                return Function(R).assign(1) * ufl.dx

            return qoi

        with self.assertRaises(ValueError) as cm:
            get_qoi(self.solver("end_time"), 0)
        msg = "Expected qoi_type to be 'time_integrated', not 'end_time'."
        assert str(cm.exception) == msg

    def test_annotate_qoi_2args_error(self):
        @annotate_qoi
        def get_qoi(solver, i):
            R = FunctionSpace(solver.meshes[i], "R", 0)

            def qoi(t, r):
                return Function(R).assign(1) * ufl.dx

            return qoi

        with self.assertRaises(ValueError) as cm:
            get_qoi(self.solver("time_integrated"), 0)
        assert str(cm.exception) == "QoI should have 0 or 1 args, not 2."

    def test_annotate_qoi_not_steady(self):
        with self.assertRaises(ValueError) as cm:
            self.solver("steady")
        msg = "QoI type is set to 'steady' but the time partition is not steady."
        assert str(cm.exception) == msg

    def test_annotate_qoi_steady(self):
        time_interval = TimeInterval(1.0, [1.0], Field("field", family="Real"))
        with self.assertRaises(ValueError) as cm:
            AdjointSolver(time_interval, self.mesh_seq, qoi_type="end_time")
        msg = "Time partition is steady but the QoI type is set to 'end_time'."
        assert str(cm.exception) == msg

    def test_qoi_type_error(self):
        with self.assertRaises(ValueError) as cm:
            self.solver("qoi_type")
        msg = (
            "QoI type 'qoi_type' not recognised."
            " Choose from 'end_time', 'time_integrated', or 'steady'."
        )
        assert str(cm.exception) == msg

    # FIXME: This was added in test/test_solver.py
    # def test_qoi_notimplemented(self):
    #     with self.assertRaises(NotImplementedError) as cm:
    #         self.solver("end_time").get_qoi(0)
    #     assert str(cm.exception) == "'get_qoi' is not implemented."

    def test_qoi_convergence_values_lt_2(self):
        solver = self.solver("end_time")
        solver.fp_iteration = solver.params.miniter + 1
        solver.qoi_values = [1.0]
        self.assertFalse(solver.check_qoi_convergence())

    def test_qoi_convergence_values_lt_miniter(self):
        solver = self.solver("end_time")
        solver.fp_iteration = solver.params.miniter
        solver.qoi_values = np.ones(solver.params.miniter - 1)
        self.assertFalse(solver.check_qoi_convergence())

    def test_qoi_convergence_values_constant(self):
        solver = self.solver("end_time")
        solver.fp_iteration = solver.params.miniter
        solver.qoi_values = np.ones(solver.params.miniter + 1)
        self.assertTrue(solver.check_qoi_convergence())

    def test_qoi_convergence_values_not_converged(self):
        solver = self.solver("end_time")
        solver.fp_iteration = solver.params.miniter
        solver.qoi_values = np.ones(solver.params.miniter + 1)
        solver.qoi_values[-1] = 100.0
        self.assertFalse(solver.check_qoi_convergence())
